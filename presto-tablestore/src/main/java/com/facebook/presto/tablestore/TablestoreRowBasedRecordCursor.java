        ...
        implements RecordCursor
{
            ...
            List<TablestoreColumnHandle> columnHandles)
    {
        ...
        WrappedRowIterator x = fetchFirst();
        rowIterator = x.getIterator();
        apiNano += x.getApiCostNanos();
    }
    ...
    private ColumnValue locateCvNonnull(int field)
    {
        ...
        List<Column> x = current.getColumn(name);
        checkArgument(!x.isEmpty(), "Can't get any column value of the field[index=%s,name=%s]", field, name);
    ...
    private Optional<ColumnValue> locateCvNullable(int field)
    {
        String name = getColumnName(field);
        List<Column> x = current.getColumn(name);
        return x.isEmpty() ? Optional.empty() : Optional.ofNullable(x.get(0).getValue());
    }
    ...
    public boolean isNull(int field)
    {
        return callAndRecord(field, () -> {
            ...
            Optional<ColumnValue> o = locateCvNullable(field);
            return !o.isPresent() || o.get().equals(INTERNAL_NULL_VALUE);
        });
    }
}
