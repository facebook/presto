        ...
        implements RecordCursor
{
    ...
    protected int rows;
    protected long apiNano;
    protected long readNano;

    public TablestoreAbstractRecordCursor(TablestoreFacade tablestoreFacade, ConnectorSession session, TablestoreSplit split,
    ...
    protected <T> T callAndRecord(int field, Callable<T> call)
    {
        ...
        try {
            ...
            throw new RuntimeException("row[pk=" + currentPrimaryKeyAsDebug() + "] field[index=" + field + "] -> " + t.getMessage(), t);
        }
        finally {
    ...
    {
        return apiNano / NANOS_PER_MILLI_SECOND;
    }
    ...
    {
        return columnHandles[idx].getColumnName();
    }

    protected boolean isPrimaryKey(int idx)
    {
        return columnHandles[idx].isPrimaryKey();
    }
    ...
    protected abstract String currentPrimaryKeyAsDebug();

    protected String toString4Value(Object value)
    {
        if (value == null) {
            return "null";
        }
        else if (value instanceof byte[]) {
            String x = Base64.getEncoder().encodeToString((byte[]) value);
