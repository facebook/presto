...
public class QueryTracker<T extends TrackedQuery>
{
    ...
    private void enforceTimeLimits()
    {
        for (T query : queries.values()) {
            ...
            Optional<QueryLimit<Duration>> queryMaxExecutionTime = getMinimum(
                    createDurationLimit(getQueryMaxExecutionTime(query.getSession()), QUERY),
                    query.getResourceGroupQueryLimits().flatMap(ResourceGroupQueryLimits::getPerQueryExecutionTimeLimit).map(
            ...
            if (executionStartTime.isPresent() && queryMaxExecutionTime.isPresent() && executionStartTime.get().plus(queryMaxExecutionTime.get().getLimit().toMillis()).isBeforeNow()) {
                query.fail(
                        new PrestoException(EXCEEDED_TIME_LIMIT,
                                format(
                                        "Query exceeded the maximum execution time limit of %s defined at the %s level",
                                        queryMaxExecutionTime.get().getLimit(),
                                        queryMaxExecutionTime.get().getLimitSource().name())));
            }
