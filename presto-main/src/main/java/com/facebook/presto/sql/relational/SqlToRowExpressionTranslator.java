...
public final class SqlToRowExpressionTranslator
{
            ...
            extends AstVisitor<RowExpression, Void>
    {
        ...
        {
            // TODO remove
            TypeWithName returnSemanticType = types.get(NodeRef.of(node));

            Type returnType = getType(node);
            Optional<Object> maybeEnumLiteral = tryResolveEnumLiteral(node, returnSemanticType);
            if (maybeEnumLiteral.isPresent()) {
                return constant(maybeEnumLiteral.get(), returnType);
            }

            Type baseType = getType(node.getBase()).getType();
